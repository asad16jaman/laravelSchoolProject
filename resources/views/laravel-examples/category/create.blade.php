                        ...
                        <div class="card-body">
                            <form method='POST' action='{{ route('add.category') }}' class='d-flex flex-column align-items-center'>
                                ...
                                <div class="form-group col-12 col-md-6 mt-3">
                                    ...
                                    <select class="form-select border border-2 p-2" name="parent_id" data-style="select-with-transition" title="" data-size="100" id="category_id">
                                        <option value="0">-</option>
                                        @foreach ($categories as $category)
                                            <option value="{{ $category->id }}">{{ $category->name }}</option>
                                            @foreach ($category->childrenCategories as $childCategory)
                                                @include('laravel-examples.category.subcategory', ['child_category' => $childCategory])
                                            @endforeach
                                        @endforeach
                                    </select>
                                    @error('parent_id')
