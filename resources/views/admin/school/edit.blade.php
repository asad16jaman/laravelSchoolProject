...
<x-auth.navbars.sidebar activePage="schools-create" activeItem="sales" activeSubitem=""></x-auth.navbars.sidebar>
                                    ...
                                    <div class="col-lg-6">
                                        <div class="">
                                            ...
                                            <select class="form-select border px-3 @error('region') is-invalid @enderror" name="region" id="region" aria-label="Default select example">
                                                <option value="">---Select--</option>
                                                <option value="A" @selected(old('region') == 'A'|| $school->region == 'A')>A</option>
                                                <option value="B" @selected(old('region') == 'B'|| $school->region == 'B')>B</option>
                                            </select>
                                            @error('region') 
                                                <p class="text-danger" style="font-size:12px">{{$message}}</p>
                                             @enderror
    ...
    @push('js')
        ...
        <script>
                ...
                let option = null;
                   ...
                   let mm = "{{ old('district') ?? false }}"
                    ...
                    option = ar.map(function(ele){
                            ...
                            return `<option value="${ele}" ${(ele == mm || "{{ $school->district }}" == ele ) ? 'selected' : ""}>${ele}</option>`
                        })
           ...
           @if (old('region') == "A" || $school->region == 'A')
            districtOption('A')
           @elseif(old('region') == "B" || $school->region == "B")
            districtOption('B')
           @endif
